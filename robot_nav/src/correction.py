...
def fnParking():
    global is_sequence_finished,current_parking_sequence,is_do_correction,pub_stop,is_robot_stop
...
def fnSeqParking():
    global desdesired_angle_turn,marker_2d_pose_y,marker_2d_pose_x,is_sequence_finished
    is_sequence_finished = True
    robot_turn = math.atan2(marker_2d_pose_y - 0, marker_2d_pose_x - 0)
    line_y_right_left = marker_2d_pose_y - 0
    fnTrackMarker(robot_turn)
    print(line_y_right_left,robot_turn)
    
    #print (marker_2d_pose_x)
    if abs(marker_2d_pose_x) < 1.25:
        fnStop()
        return True
    else:
        return False
...
if __name__ == '__main__':
    ...
    sub_info_marker = rospy.Subscriber('/camera_front_ar_pose_marker', AlvarMarkers, cbGetMarkerOdom, queue_size = 1)
    sub = rospy.Subscriber("/camera_front_ar_pose_marker", AlvarMarkers, newOdom, queue_size = 1)
    ...
    while not rospy.is_shutdown():
        if is_do_correction == True and is_odom_received == True : #and is_robot_stop == True:
            fnParking()
