...
def middle():
    global x_ar,w_ar,ar_tak,is_at_middle
    if x_ar > 0.1:
        desired_angle_turn = 0.1
...
def fnParking():
    global is_sequence_finished,current_parking_sequence,is_do_correction,is_at_middle,is_robot_stop
    if current_parking_sequence == ParkingSequence.searching_parking_lot.value:
        ...
        if is_sequence_finished == True:
            ...
            if is_sequence_finished == True:
                print ("Finished 5")
                current_parking_sequence = ParkingSequence.tur.value
                is_sequence_finished = False
...
def ft():
    global x_ar,w_ar,ar_tak
    if x_ar > 0.00:
        desired_angle_turn = 0.01
        fnrt(desired_angle_turn)
        return False
    if x_ar < -0.003:
        desired_angle_turn = -0.01
...
def fnSeqParking2():
    ...
    fnTrackMarker2(line_y_right_left)
...
def fnSeqParking():
    global desdesired_angle_turn,marker_2d_pose_y,marker_2d_pose_x,is_sequence_finished,y_ar
    is_sequence_finished = True
    line_y_right_left = math.atan2(marker_2d_pose_y - 0, marker_2d_pose_x - 0)
    #fnTrackMarker(line_y_right_left)
    print(line_y_right_left)
    if line_y_right_left > 1.54:
        fnTrackMarker(line_y_right_left * -1)
    else:
        fnTrackMarker(line_y_right_left)
...
def fnSeqChangingDirection():
        global desired_angle_turn,marker_2d_pose_y,marker_2d_pose_x,y_ar
        desired_angle_turn = -1. *  math.atan2(marker_2d_pose_y - 0, marker_2d_pose_x - 0)
        print(desired_angle_turn)
        if abs(desired_angle_turn) < 1.55:
            fnTurn(-desired_angle_turn) 
        elif abs(desired_angle_turn) > 1.58:
            fnTurn(desired_angle_turn)   
        elif abs(desired_angle_turn) < 1.58 and abs(desired_angle_turn) > 1.55:
            fnStop()
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    sub_odom_robot = rospy.Subscriber('/odom_ekf', Odometry, cbGetRobotOdom, queue_size = 1)
    sub_info_marker = rospy.Subscriber('/camera_left_ar_pose_marker', AlvarMarkers, cbGetMarkerOdom, queue_size = 1)
    sub = rospy.Subscriber("/camera_left_ar_pose_marker", AlvarMarkers, newOdom, queue_size = 1)
    sub_corr = rospy.Subscriber("/camera_number", String, ccc, queue_size = 1)
    sub_baselink_turn = rospy.Subscriber("/robot_stop", String, sss, queue_size = 1)
    ...
    while not rospy.is_shutdown():
        if is_at_middle ==False and is_do_correction == True : #and is_robot_stop == True:
            middle()
            loop_rate.sleep()
        elif is_do_correction == True and is_at_middle == True and is_odom_received == True :
