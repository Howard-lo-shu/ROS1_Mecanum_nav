...
def fnSeqChangingDirection():
        ...
        fnTurn(desired_angle_turn)       
        if abs(desired_angle_turn) < 1.60:
            fnStop()
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    sub_info_marker = rospy.Subscriber('/camera_right_ar_pose_marker', AlvarMarkers, cbGetMarkerOdom, queue_size = 1)
    sub = rospy.Subscriber("/camera_right_ar_pose_marker", AlvarMarkers, newOdom, queue_size = 1)
    ...
    while not rospy.is_shutdown():
        if is_at_middle ==False and is_do_correction == True:
            middle()
            loop_rate.sleep()
        if is_do_correction == True and is_at_middle == True and is_odom_received == True : #and is_robot_stop == True:
