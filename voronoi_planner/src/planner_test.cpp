...
void VoronoiPlanner::initialize(std::string name, costmap_2d::Costmap2D* costmap, std::string frame_id) {
    if (!initialized_) {
        ...
        ros::Subscriber costmapUpdateSubscriber = private_nh.subscribe("/move_base_flex/global_costmap/costmap_updates", 10, &VoronoiPlanner::costmapUpdateCallback, this);
        // ros::Subscriber mapSubscriber=private_nh.subscribe("/map",1,&VoronoiPlanner::mapCallback, this);
